An algorithm to test the reliability of the 3D phase unwrapping method built in StaMPS using synthetic data. Note that the code is designed for linear (temporally) velocity field. You may modified the code according for non-linear cases. 

This code comes along with the paper: Fanghui Deng, Mark Zumberge. Seafloor motion from offshore platforms using satellite radar images – a case study in the Adriatic Sea. Under revision. 

The code was written and tested in Python 3.10.12 under Ubuntu Ubuntu 22.04. The following are steps of how to use the code. 
...
1. Prepare the following files manually or use your own codes: xy.txt, day.txt, ifgday_ix.txt, bperp.txt, vel_real.txt. Examples files were given. 
	...
	vel_real.txt: N x 1 vector giving displacement rate (in InSAR LOS direction) of each PS point, unit: mm/year.
